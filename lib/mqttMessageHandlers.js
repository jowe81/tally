...
const handleTimerUpdate = (io, topic, msg) => {
  if (msg && msg.data && msg.data.remaining !== undefined) {
    //Attach color for client to display depending on remaining time
    let color = "red";
    if (msg.data.remaining >= constants.TIMER_GREEN) {
      color = "green";
    } else if (msg.data.remaining >= constants.TIMER_ORANGE) {
      color = "orange";
    }
